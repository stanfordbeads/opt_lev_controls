...
def main():
    '''
    adapted from https://gist.github.com/Mallekin/14b83dce512e1f67e2a0fdb0499f6183
    '''
...
# max width = 752
# max height = 480
widthdummy = round(752*.2)
heightdummy = round(480*.2)
rect_aoi = ueye.IS_RECT()
rect_aoi.s32X = ueye.int(0)
print(ueye.int(0))
rect_aoi.s32Y = ueye.int(0)
...
#nRet = ueye.is_AllocImageMem(hCam, widthdummy, heightdummy, nBitsPerPixel, pcImageMemory, MemID)
#if nRet != ueye.IS_SUCCESS:
...
mem_ptr = ueye.c_mem_p()
print(mem_ptr)
ret = ueye.is_AllocImageMem(hCam, widthdummy, heightdummy, nBitsPerPixel,
                                mem_ptr, MemID)
# set active memory region
ret = ueye.is_SetImageMem(hCam, mem_ptr, MemID)
print(f"SetImageMem returns {ret}")
